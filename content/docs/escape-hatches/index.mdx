...
* [Как "запоминать" информацию без повторного рендера](/docs/escape-hatches/referencing-values-with-refs)
* [Как получать доступ к DOM-элементам, которыми управляет React](/docs/escape-hatches/manipulating-the-dom-with-refs)
* [Как синхронизировать компоненты с внешними системами](/docs/escape-hatches/synchronizing-with-effects)
* [Как удалить ненужные Effects из компонентов](/docs/escape-hatches/you-might-not-need-an-effect)
* [Чем lifecycle Effect отличается от lifecycle компонента](/docs/escape-hatches/lifecycle-of-reactive-effects)
* [Как не дать некоторым значениям повторно запускать Effects](/docs/escape-hatches/separating-events-from-effects)
* [Как сделать так, чтобы Effect запускался реже](/docs/escape-hatches/removing-effect-dependencies)
* [Как делиться логикой между компонентами](/docs/escape-hatches/reusing-logic-with-custom-hooks)

</Callout>

## Хранение значений с помощью ref {/*referencing-values-with-refs*/}

Когда вам нужно, чтобы компонент "помнил" какую-то информацию, но вы не хотите, чтобы эта информация [запускала новые рендеры](/docs/adding-interactivity/render-and-commit), вы можете использовать *ref*:
...
import { Example110 } from '../../../examples/example copy 110';
